...
# try `geneIndex <- loadObject(file = url(ExampleIndex_TabulaMuris10X))` for another option of the Tabula Muris Datasets
...
```{r}
result[which.max(result$tfidf),]

bestQuery <- strsplit(as.character(result[which.max(result$tfidf),c('Query')]), ',')[[1]]
bestQuery
```
